...
import { createCommand } from "../../services/commandeService";
import StripePayment from "./StripePayment";
const Cart = () => {
  const [cartdata, setcartdata] = useState(getCartFromLocalStorage);
  ...
  const handleAddCommand = async () => {
    ...
    const response = await createCommand(dateLivraison, addresse, phoneNumber);
    setCommandId(response._id);
  ...
  const fetchAddresses = async () => {
    ...
    try {
      const response = await getData(
        `http://localhost:5001/users/${userId}/addresses`
      );
  ...
  const calculateCardTotal = () => {
    if (cartdata.length === 0) {
      ...
      for (let item of cartdata) {
        total = total + item.product.prix * item.quantity;
      }
  ...
  const removeOneFromCart = (product) => {
    decrementQuantityFromCart(product);
    setreloadnavbar((prev) => !prev);
      ...
      <div className="cart">
              ...
              <table className="carttable">
                ...
                <tbody>
                      ...
                      <tr key={index} className="cartitemrow">
                        ...
                        <td data-label="Quantity">
                          <div className="quantity">
                            <button
                              className="minus"
                              onClick={() => removeOneFromCart(item.product)}
                            >
                              -
                            </button>
                        ...
                        <td>
                          <p>
                            DT {(item.product.prix * item.quantity).toFixed(2)}
                          </p>
                        </td>
                  ...
                  <tr>
                    ...
                    <td className="totaltableright">{calculateCardTotal()} </td>
                  </tr>
                </tbody>
              </table>
            ) : (
        ...
        {active == 2 && (
          <div className="shippingcont">
            ...
            <div className="selectdate">
              <h2 className="mainhead1">Your Phone number </h2>
              <input
                type="text"
                value={phoneNumber}
                onChange={(e) => {
        ...
        {active == 3 && (
          <StripePayment commandId={commandId} setactive={setactive} />
        )}
        {active == 4 && (
        ...
        {active == 4 && (
          <div className="btns">
            ...
            >Back</button> */}
            <a className="nextbtn" href={`/${userRole}/yourorders`}>
              Check commands
            </a>
          </div>
        )}
